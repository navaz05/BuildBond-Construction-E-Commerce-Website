...
// Get category parameter (should always be 'product' for this page)
$category = isset($_GET['category']) ? mysqli_real_escape_string($con, $_GET['category']) : 'product';

// Modify SQL query based on search or specific product
if ($productId > 0) {
    // If a specific product is requested
    $sql = "SELECT * FROM products WHERE id = $productId AND category = 'product'";
    ...
    $sql = "SELECT * FROM products WHERE category = 'product' AND (name LIKE '%$search%' OR description LIKE '%$search%')";
    $result = mysqli_query($con, $sql);
} else {
    // Default query - all product products
        ...
        <div class="col-12">
            ...
            <div class="row" id="productsContainer">
                ...
                if ($result && mysqli_num_rows($result) > 0) {
                    ...
                    echo '<div class="col-12 text-center"><h3>No products found matching your search.</h3></div>';
                }
                ?>
            </div>
        </div>
